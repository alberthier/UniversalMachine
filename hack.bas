...
CDXC     DIM digitscount AS INTEGER
CDXCV    digitscount = X
D        DIM digits(digitscount) AS STRING
DV       digits(I) = "1"
DX       digits(II) = "2"
DXV      digits(III) = "3"
DXX      digits(IV) = "4"
...
DXXX     digits(VI) = "6"
DXXXV    digits(VII) = "7"
DXXXVI   digits(VIII) = "8"
DXXXVII  digits(IX) = "9"
DXXXVIII digits(X) = "0"
DXL      DIM j AS INTEGER
DXLV     DIM k AS INTEGER
DL       DIM pass AS STRING
DLV      i = I
DLX      j = I
DLXV     k = I
DLXX     pass = words(i) + digits(j) + digits(k)
DLXXV    IF CHECKPASS(username, pass) THEN GOTO DCXL
DLXXX    REM PRINT "trying " + pass + CHR(X)
DLXXXV   k = k + I
DXC      IF k > digitscount THEN GOTO DC
DXCV     GOTO DLXX
DC       j = j + I
DCV      k = I
DCX      IF j > digitscount THEN GOTO DCXX
DCXV     GOTO DLXX
DCXX     i = i + I
DCXXV    j = I
DCXXX    IF i > pwdcount THEN GOTO DCL
DCXXXV   GOTO DLXX
DCXL     PRINT "found match!! for user " + username + CHR(X)
DCXLV    PRINT "password: " + pass + CHR(X)
DCL      END
